(function($) {
  ...
  StyleHatch.Header = (function() {
    var selectors = {
      ...
      turnpupMobileMenuClose: '.mobile-icon-close',
    ...
    function cacheSelectors() {
      cache = {
        ...
        //TURNPUP Add Headers
        $turnpupSearchLink: $(selectors.util).find('a#turnpup_search'),
        $turnpupSearchLinkIcon: $(selectors.util).find('a#searchIcon'),
        $inlineSearch: $(selectors.util).find('.search-nav-item'),
        $inlineSearchClose: $(selectors.util).find('.button-wrapper'),
    ...
    function initUtilHeader() {
      ...
      if (!$(selectors.menuOverlay).length) {
        ...
        // turnpup
        cache.$turnpupMobileMenuClose = $(selectors.turnpupMobileMenuClose);
      }
     ...
     */
    function openSearch() {
      cache.$inlineSearch.css('display','flex');
      cache.$inlineSearchClose.css('display','flex');      
      //turnpup coment out below because of above
...
//       });
    ...
    function closeSearch() {
      ...
      cache.$inlineSearch.css('display','none');
      cache.$inlineSearchClose.css('display','none');
      //turnpup coment out below because of above
//       cache.$searchInput.blur();
//       clearTimeout(config.blurTimer);
...
//       });
    ...
    function bindUtilHeaderEvents() {
      // Search
      //TURNPUP start
      cache.$turnpupSearchLink.on('click.search', function(e) {
        openSearch();
        e.preventDefault();
      });
      cache.$turnpupSearchLinkIcon.on('click.search', function(e) {
        console.log("clicked search");
        if(!cache.$turnpupIsSearchOpen)
        {
          openSearch();
          cache.$turnpupIsSearchOpen = true;
        } else {
          closeSearch();
          cache.$turnpupIsSearchOpen = false;
        }        
        e.preventDefault();
      });
      //TURNPUP end
      ...
      cache.$inlineSearchClose.on('click.search', function(e) {
        console.log("inlineSearchClose");
        closeSearch();
        e.preventDefault();
      });
      ...
      cache.$turnpupMobileMenuClose.on('click.panel', function(e) {
        togglePanelMenu();
        e.preventDefault();
      });
      ...
      cache.$menuPanelDropdown
        ...
        .on('click.panelDropdown', function(e) {
          if (
            $(this) // turnpup was --> !$(this)
              .closest('li')
              .hasClass('has-dropdown') // turnpup was --> .hasClass('expanded')
          ) {
            e.preventDefault();
          }
        });
      ...
      cache.$dropdownParent.on(
        ...
        function(e) {
          ...
          if (!$el.hasClass(config.dropdownActiveClass)) {
            e.preventDefault();
            showDropdown($el);
            // turnpup add 2 lines below
//             console.log("clear timeout");
//             clearTimeout(turnpupTimer); //turnpup add
          }
        }
      );
      
      cache.$dropdownParent.on('mouseleave.dropdown', function() {
        //turnpup add below block
//         (function(e){ //turnpup add
//         turnpupTimer = setTimeout(function() { //turnpup add
//           hideDropdown(e); }, 2500 //turnpup add
//          ) //turnpup add
//         })($(this)); //turnpup add
        hideDropdown($(this));
      });
  ...
  StyleHatch.Product = (function() {
    function Product(container) {
      ...
      this.selectors = {
        ...
        turnpupInventoryAvailability: '#tp-available-inventory', //  turnpup inventory availability selector
        SKU: '.variant-sku',
    ...
    Product.prototype = $.extend({}, Product.prototype, {
      _initVariants: function() {
        ...
        this.$container.on(
          ...
          this._turnpupUpdateInventoryQuantity.bind(this)
        );
...
//         this._turnpupUpdateInventoryQuantity(evt); 
      ...
      // turnpupGoHere
      // turnpup invetory availability update inventory availability 
      _turnpupUpdateInventoryQuantity: function(evt) {
        var inventory_qty = inv_qty[evt.variant.id];
        var updateString = "";
        if (inventory_qty <= 0) {
          updateString = StyleHatch.Strings.outOfStock + ": " + inventory_qty;
        } else if ( inventory_qty <= 50 ) {
          updateString = StyleHatch.Strings.leftInStock + ": " + inventory_qty;
        } else {
          updateString = StyleHatch.Strings.inStock + ": " + inventory_qty; 
        }
        console.log("3998 theme.js === function to actually update inventory  -->  " + inventory_qty );
        $(this.selectors.turnpupInventoryAvailability).html(
          updateString
        );
      },
  ...
  slate.Variants = (function() {
    ...
    Variants.prototype = $.extend({}, Variants.prototype, {
      ...
      /** turnpupGoHere
      TURNPUP 
       * Trigger event when variant inventory quantity changes.
       *
       * @param  {object} variant - Currently selected variant
